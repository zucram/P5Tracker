...
export function Planner({ state, commit, selectTab }) {
  ...
  const activity = target && activities.activities.find(item => item.rewards[target.statGate.stat.toLowerCase()] && item.days.includes(new Date(dateNumber(state.date) * 86400000).getUTCDay() || 7) && item.slots.includes(state.slot) && !item.requirements);
  const priorities = getMonthGuide(state.month).strategy;
  return <section aria-labelledby="planner-title" className="planner">
    ...
    {!plan.blocks.length && <section className="planner-section"><h3>Options to check {state.slot === 'daytime' ? 'today' : 'tonight'}</h3><p className="small-note">Based on usual weekdays, reviewed closures and your recorded introductions. Story interruptions, invitations and rank-up readiness still need checking in-game.</p>
      <div className="choice-grid">
        ...
        <article className="choice-card"><span className="event-category">Flexible alternative</span><h4>{target ? `Build ${target.statGate.stat}` : 'Leave room for recovery'}</h4><p>{target ? `${target.arcana} needs ${target.statGate.stat} rank ${target.statGate.rank} to start; you have rank ${state.stats[target.statGate.stat]}. ${activity ? `${activity.title} at ${activity.venue} offers +${activity.rewards[target.statGate.stat.toLowerCase()]} points for ${activity.costYen ? `¥${activity.costYen.toLocaleString('en-US')}` : 'free'} on this weekday.` : 'Check the stat activity reference for an available option.'}` : 'Use a free slot for affinity, resources or another unfinished goal. The planner does not force a perfect-run route.'}</p><button onClick={() => selectTab('briefing')}>Compare stat activities</button></article>
      </div>
    ...
    <section className="planner-section"><h3>Your {plan.month?.name} priorities</h3><p className="small-note">Flexible guidance for the month, separate from hard deadlines.</p><ul className="monthly-priorities">{priorities.map(task => <li key={task.id}>{task.detail}</li>)}</ul><button onClick={() => selectTab('calendar')}>Add your own monthly goals</button></section>
