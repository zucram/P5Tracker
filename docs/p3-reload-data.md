...
`src/p3/data.js` describes the main campaign of Persona 3 Reload. It does not cover Persona 3, FES, Portable, or Episode Aigis. Initial roster review took place on 7 September 2026. The prototype is unpublished pending the sourced semi-guide. See [the knowledge review](p3-reload-knowledge-review.md) for the broader research and release gaps.
...
The month list is a set of planning buckets. It does not claim that the entire story ends in January. The module contains no daily schedules, required monthly rank targets, rescue deadlines, romance choices, Linked Episodes, or guarantee of completing every Social Link in one run.
