...
All 110 automated tests passed. Isolated Chromium checks at 390 and 1280px verified tab order, Space/Enter, repeat suppression, pointer toggles, saved Persona registration, exact school dates and result counts, calendar/month controls, Palace seeds/Personas and Mementos requests. Seeded Reload state remained unchanged. Royal's changed modules now pass lint after removing a dead helper and resolving two unused-binding reports. Full-repository lint still reports seven pre-existing errors and one warning in other files; this release does not claim a clean repository-wide lint run.


## Treasure Demon guide and registry in 2.6.15

`/guides/persona-5-royal-treasure-demon-weaknesses/` puts all nine Royal weaknesses in a compact table with name, damage-type and level lookup. The guide distinguishes Royal from the original game's affinities, explains recruitment and links to the Royal fusion calculator. Names, levels and weaknesses were checked against two references; it does not publish disputed spawn-location advice. Source details are in [Royal content review](royal-content-review.md).

The tracker CTA opens `/p5/?collection=treasure#registry`. The fixed query parameter initializes a Treasure Demons-only filter; it does not import or change progress. The filter can be cleared to show the full registry. Each rare Persona displays its weakness and exposes it as an accessible description. Name search accepts apostrophe/hyphen-free aliases. Existing `p_<name>` save keys and the overall registry completion denominator remain unchanged.

The directory, Royal Registry/Metaverse navigation and sitemap link the page. Its own share image brings coverage to 17 sitemap pages and 13 guides. All 111 automated tests and build checks passed. Isolated 320/390/1280px browser tests cover the nine-entry table, aliases and numeric lookup, empty/reset states, no-JavaScript content, fixed-slug analytics, the filtered-registry handoff, keyboard capture and preserved Royal/Reload data. Search terms remain local. Incremental traffic, ranking and revenue remain unmeasured.


## Royal request and activity review in 2.6.16

Legacy Mementos entries assigned several requests to the wrong paths or confidants and displayed incorrect rewards. The reviewed selection now contains twenty requests, including Kaitul, four January requests and a separate Sojiro visit. Thirteen existing request IDs remain attached to their corrected entries. The unverified January placeholder is retired; its stored key is preserved without completing any new request. Earlier paths remain visible because confidant requests can arrive later, and headers show recorded completion counts instead of unreviewed enemy-level targets. An in-app scope note explains original locations and possible third-semester relocation.
