...
## Calendar activities in 2.6.16

The [Royal calendar](https://megatenwiki.com/wiki/Calendar_in_Persona_5_Royal) places the Hawaii hangout on September 10 and the post-festival hangout on October 26. These correct September 19 and October 30 while retaining `s3` and `o3`. The [Royal walkthrough](https://aqiu384.github.io/megaten-database/p5r/ace-walkthrough) identifies Justice rank 1 as automatic daytime story progress on June 10, replacing the incorrect night visit in Kichijoji.

Kawakami rank 10 before summer is now an early planning goal rather than a critical July 24 deadline. The July 24 library claim is removed in favor of borrowing before the break; this review does not establish an exact last borrowing date. Calendar story locks around July 20–24 make the previous advice unsafe. The existing July book and Kawakami IDs remain stable. `j5` still refers to The Money-grubbing Uncle, but now identifies Sojiro/Hierophant and sits in September with the rank-8 prerequisite instead of pretending to be a July Kawakami lock.

## Selected Mementos requests in 2.6.16

`royalMementos.js` owns twenty selected requests, grouped by their original paths plus Sojiro's special visit. This is not a complete request list. Three references informed the review: [Megami Tensei Wiki's Royal table](https://megatenwiki.com/wiki/Requests_in_Persona_5_Royal), [marendarade's Royal Mementos reference](https://gamefaqs.gamespot.com/ps4/260936-persona-5-royal/faqs/78212/mementos), and [NightlyGamingBinge's request walkthroughs](https://nightlygamingbinge.com/persona-5-royal-mementos-requests-guide/). The first page contains both editions; the Royal table was used.

Corrections preserve all thirteen existing real-request IDs. Rewards corrected include Attachment Pearl, Protein, Cat Brooch, Old Key, Whip Sword, Charge, Null Phys, High Counter, Envy Chain, Soma and Venture Saber. Sadism moves to Aiyatsbus; Kawakami's corrected A Teacher Maid to Suffer moves to Chemdah; Who's Been Assaulting People moves to Kaitul; Winners Don't Use Cheats and The Killer Who Cleans Up Trash move to Akzeriyyuth. A Mother's Aggression belongs to Shinya/Tower in Adyeshach, while Futaba's separate Daughter's Just a Meal Ticket receives a new ID. The Money-grubbing Uncle is Sojiro/Hierophant progress and uses a special visit initiated through Futaba, rather than a normal full-party search.
